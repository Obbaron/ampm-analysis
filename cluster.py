...
def assign_parts(
    clustered_data: np.ndarray, parts_df: pd.DataFrame, save_file: bool = True
) -> np.ndarray:
    """
    ...
    if len(cluster_to_part_map) != n_clusters:
        ...
        raise ValueError("Could not map all clusters to parts")
...
data = import_ampm_data(filepath=data_directory, start_layer=101, end_layer=150)
